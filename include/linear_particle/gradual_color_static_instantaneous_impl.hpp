...
str linear_particle::gradual_color_static_instantaneous(
    ...
    A << 1, tbe(0), 1, tbe(1);
    b << from(0), from(1), from(2), to(0), to(1), to(2);
    x = A.colPivHouseholderQr().solve(b);

    str_stream ssin;
    ssin << std::format(" x = ({:{}.{}f}) + ({:{}.{}f}) * t; ", x(0, 0), length,
                        prec, x(1, 0), length, prec)
         << std::format(" y = ({:{}.{}f}) + ({:{}.{}f}) * t; ", x(0, 1), length,
                        prec, x(1, 1), length, prec)
         << std::format(" z = ({:{}.{}f}) + ({:{}.{}f}) * t; ", x(0, 2), length,
                        prec, x(1, 2), length, prec);
